...
def send_booking_confirmation_email(booking_id: int):
    """
    Send booking confirmation email to user when a new booking is created
    ...
    try:
        ...
        # Email context
        subject = f'Booking Created - {booking.listing.title}'
        ...
        <p>Your booking is currently <strong>{booking.get_status_display().lower()}</strong>. You will receive another email once payment is completed.</p>
        ...
        logger.info(f"Booking confirmation email sent to {booking.user.email} for booking {booking_id}")
